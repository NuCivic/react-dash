...
export default class DataTable extends BaseComponent {
  ...
  getPages(size, total, current) {
    const totalPages = this.getTotalPages(size, total);
    const displayedPages = 10;
    const centroid = displayedPages / 2;
    let start;
    let end;
  ...
  getLastPage(size, total) {
    const pages = this.getTotalPages(size, total);
  ...
  _onPageChange(e, id, mouseEvent, pageNumber) {
    e.preventDefault();
    const rpp = this.state.rowsPerPage;
    const query = Object.assign({}, this.state.queryObj, {
      size: rpp,
  ...
  render() {
    ...
    const tableDefaultProps = getProp('settings.table', this.props);
    const columnDefaultProps = getProp('settings.columns', this.props);
    ...
    const rowDefaultProps = getProp('settings.rows', this.props);
    const headers = Object.keys(data[0] || {});
    const totalPages = this.getTotalPages(this.state.rowsPerPage, this.state.total);

    // Create the colums
    const columns = headers.map((header, headerIndex) => {
      const columnOverrides = getProp(`columns. ${header}`, this.props.overrides);
      return (
        <Column
          ...
          flexGrow={1}
          cell={(props) => {
            const rowOverrides = getProp(`rows.${props.rowIndex}`, this.props.overrides);
            const cellOverrides = getProp(`cells.${header}_${props.rowIndex}`, this.props.overrides);
            return (
              <Cell
                {...props}
                {...columnOverrides}
